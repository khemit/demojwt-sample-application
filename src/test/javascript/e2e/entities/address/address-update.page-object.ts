...
export default class AddressUpdatePage {
  pageTitle: ElementFinder = element(by.id('demojwtApp.address.home.createOrEditLabel'));
  ...
  async enterData() {
    ...
    expect(await this.getAddress1Input()).to.match(/address1/);
    await waitUntilDisplayed(this.saveButton);
    await this.setAddress2Input('address2');
    expect(await this.getAddress2Input()).to.match(/address2/);
    await waitUntilDisplayed(this.saveButton);
