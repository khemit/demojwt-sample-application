...
export default class WishListUpdatePage {
  pageTitle: ElementFinder = element(by.id('demojwtApp.wishList.home.createOrEditLabel'));
  ...
  async enterData() {
    ...
    if (selectedRestricted) {
      ...
      expect(await this.getRestrictedInput().isSelected()).to.be.false;
    } else {
      await this.getRestrictedInput().click();
      expect(await this.getRestrictedInput().isSelected()).to.be.true;
    }
